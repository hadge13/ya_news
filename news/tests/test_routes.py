...
# Импортируем класс модели новостей. Чтобы проверить доступность страницы новости, нужно эту новость создать
...
class TestRoutes(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Для проверки страница создания\удаления комментария (надо создать автора коммента, простого автора, комментарий)
...
# ) 
    ...
    # имя пути и позиционные аргументы для функции reverse().
        ...
        # id записи; передаём его в кортеже.
            ...
            ('users:login', None),      #  Страница входа в учётную запись и 
            ('users:logout', None),     # Страница выхода из неё доступны анонимным пользователям.
...
# Если бы не было общего теста на 3 пункта, вот так бы расписывали каждый тест
# # Главная страница доступна анонимному пользователю.
...
#         url = reverse('news:detail', args=(self.news.id,))    # Или kwargs={'pk'=self.news.id}
#         response = self.client.get(url)
#         self.assertEqual(response.status_code, HTTPStatus.OK) 
    ...
    # П.4 При попытке перейти на страницу редактирования или удаления комментария анонимный пользователь перенаправляется на страницу авторизации.
    # Проверка ридайректов
